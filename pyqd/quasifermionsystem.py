...
# 2. Vectorize calculation of 'diffs' in .areNeighbours(), currently uses list
#   comp.
...
class QuasiFermionicDotSystem:
    ...
    def __init__(self, verbose=True):
        ...
        self.objects = {}
        # Dictionary of possible system states, searchable by total charge N
        self.states = {}
        # Dictionary of all tunneling Hamiltonians, with N as key
        self.__tunnelingHam = {}
        # Dictionary of all orbital + mutual capacitance diagonals, w/ N as key
        self.__orbElements = {}
        # Dictionary containing properties from .__sysTemp in numpy arrays
        self.__sys = {}
        # List containing system information in mutable list format
        # Formatted as:
        #  .__sysTemp[i] = ['name',Ec,isSC,isLead,orbitals,deg.,ts,Ems]
        self.__sysTemp = []
    ...
    def __str__(self):
        """Print information when qdsystem is called with print()."""
        output = (
            'qdsystem class object with: \n'
            + str(self.ndots)+' total dot{plr}, '.format(plr='s'
                                                         if self.ndots > 1
                                                         else '')
            + str(self.ndotssc)+' of which are superconducting and '
            + str(self.ndots-self.ndotssc)+' of which are normal, as well as '
            + str(self.nleads)+' lead{plr}.'.format(plr='s'
                                                    if self.nleads > 1 else '')
        )
        return output
    ...
    @property
    def __ndotseff(self):
        """Number of effective dots, including normal dot degeneracy."""
    ...
    def __cast_as_sclr(self, n):
        ...
        if np.isscalar(n):
            ...
            raise Exception("Input must be an array-like of length 1 or a"
                            + " scalar!")
    ...
    def add_dot(self, Ec,
        ...
        orbitals (float or float list): Orbital addition energy or SC
            gap. If length is greater than one, each entry is the orbital
            energy for each successive orbital. After all orbitals are filled,
            subsequent orbital energies are assumed equal to the last orbital
            energy.
        u (float): (Even,n)-->(Odd,n+1) parity tunneling multiplier (coherence
            factor u) The corresponding v = sqrt(1-u^2) is automatically
            inferred from this. If u==0, equal electron-like and hole-like
            tunneling is assumed.
        isSC (bool): Whether or not dot is superconducting, in which case:
            --> degeneracy = degeneracy of odd parity quasiparticle state of
            dot (/2 if spin)
        ...
        if any([name == n for n in self.objects]):
            raise Exception("Dot or lead with name '" + name
                            + "' is already defined.")
        if spin:
            deg = 2*degeneracy
            degType = 'spin'
        ...
        self.__sysTemp.extend([[
            ...
            orbitals,  # Odd parity energy if isSC, else is orbital energy list
            degType,  # Degree of freedom corresponding to degeneracy or 'none'
            deg,  # Degeneracy of orbitals (QP states) for dots (SC islands).
            ...
            u,  # e-like coherence factor or no coherence factors (u == 0)
            ]]*numEffDots)
        if self.verbose:
    ...
    def add_coupling(self, Em, t, dotnames, twoe=False):
        ...
        dotnames (tuple or set of strings): List of names of dots to be
            coupled.
    ...
    def add_lead(self, dots, t, name=None, level=0):
        ...
        if any([name == n for n in self.objects]):
            raise Exception("Lead or dot with name '" + name
                            + "' is already defined.")
        self.objects[name] = {
            'couplings': {},
            'twoeCouplings': {},
        ...
        self.__sysTemp.append([name, 0, False, True, level,
                               'none', 1, {}, {}, {}, 0])
    ...
    def delete(self, *names):
        ...
        def del_coupling(*names):
            ...
            if np.isscalar(names) or len(names) == 1:
                ...
                for n in [n for n in self.objects
                          if name in self.objects[n]['couplings']]:
                    del self.objects[n]['couplings'][name]
                    self.objects[n]['numCouplings'] -= 1
                for n in [n for n in self.objects
                          if name in self.objects[n]['twoeCouplings']]:
        ...
        if len(names) == 1:
            ...
            # Dict of system states is no longer valid after object is removed
            self.states = {}
            self.__tunnelingHam = {}
            self.__orbElements = {}
        else:
    ...
    def finalize(self):
        """Port system info from .__sysTemp to dict of numpy arrays in .__sys

        Translates system information to a dictionary of numpy arrays, so that
        vectorized calculations can be carried out afterwards.
        """
        nDots = self.__ndotseff  # Number of eff. dots / leads in system
        ...
        self.__sys.update({
            ...
            'Em': {},
            't': np.zeros((nDots, nDots)),
            'twoet': np.zeros((nDots, nDots)),
            'u': np.full((nDots, 2), 1, dtype=float)
        })
    ...
    def get_states(self, N):
        """Generate possible charge/orbital/spin states for total charge N.
        ...
        Returns:
        numpy.ndarray: Contains state of each (quasi-)dot/lead in same order
            as in self.__sys. First index of each state is object charge,
            second is integer representing orbital or spin quantum number,
            last one is 1 (if spinful) or 0 (not spinful).
        """
        ...
        def EcNotEnforced(chgs, degIndices=degIndices):
            """Ensures degenerate dots have no extra charge in deg. orbital."""
            return any([any([abs(d[0]-d[1]) > 1
        ...
        def dotstates(charges):
            """Gets possible dot/lead (of index i) states for given charge."""
            # First index corresponds to charge, second to quantum number of
            # quasi-dot and third to whether or not dot is spinful.
            return [
        ...
        def dotChargeIsNotN(charges, N):
            """Returns True if dot charges are above N, ignoring leads."""
            return np.any(charges[sys['isLead'] is False] > N)

        def partitions(n, k, NInput, minPartSize=0, kc=None):
            """Yields possible charge/orbital states.

            Finds all possible partitions of n into k integers with minimum
            partition size l. kc is used for the recursive part of the
            algorithm. Augments this algorithm by generating all unique
            permutations of results, and checking which are physical for the
            system at hand.
            """
            if kc is None:
                kc = k
            ...
            if kc == 1:
                ...
                return
            for i in range(minPartSize, n+1):
                # Call partitions with l=i to ensure that no tuple is yielded
                # twice, as this enforces i being the largest integer
                for r in partitions(n-i, k, NInput, minPartSize=i, kc=kc-1):
                    s = tuple(r) + (i,)
                    ...
                    if len(s) == k:
                        ...
                        for p in [np.array(p)
                            ...
                            # Enforce charging energy if degenerate dots exist.
                            if areDegenerateDots and EcNotEnforced(p):
                                continue
                            # Enforce that dots can only have N electrons,
                            # while leads can have ndots*nleads*N electrons:
        ...
        self.states[N] = np.array([chgs
                                   for chgs in partitions(nMax, nObjs, N)])
        if self.verbose:
            print("Time to generate " + str(len(self.states[N][:, 0, 0]))
                  + " system states was " + str(time.perf_counter() - ti)
                  + 's')
        return self.states[N]

    def dimension(self, N):
        """Return dimension of Hilbert space for N charges in the system."""
    ...
    def onsite(self, states, gates, N=None):
        ...
        ns = np.array([np.sum(states[:, sys['name'] == n, 0], axis=1)
                       - gates[n]
                       for n in gs]).T
        EcTot = np.sum([sys['Ec'][sys['name'] == n][0]*ns[:, i]**2  # pylint: disable=E1136  # pylint/issues/3139
                        for i, n in enumerate(gs)], axis=0)
        EmTot = np.zeros(nStates)
        for i,n1 in enumerate(gs):
            Em = sys['Em'][n1]
            EmTot += np.sum([Em[n2]*ns[:, i]*ns[:, j]  # pylint: disable=E1136  # pylint/issues/3139
                             for j, n2 in enumerate(gs)], axis=0)/2
        # Orbital matrix element terms only need to be calculated once.
        if N is None or N not in self.__orbElements:
            # Compute orbital energy of SC quasiparticles then of electrons on
            # normal dots.
            Eorb = np.einsum(
                'i,ij',
            ...
            Eorb += np.sum(
                [
                    ...
                    for i, o in enumerate(sys['orb'][sys['isSC'] is False])
                ],
                axis=0
                )
    ...
    def are_neighbours(self, states, N=None, hamiltonian=False):
        ...
        (ndarray): Array with size equal to the number of input states squared,
            ...
            nearest neighbours, and False in all others (hamiltonian == True),
            or the tunneling Hamiltonian given couplings in .__sys['t']
            (hamiltonian == False).
        """
        if hamiltonian:
            if N is None:
        ...
        nStates = len(states[:, 0, 0])
        ti = time.perf_counter()
        # Generate array (numStates,numStates,len(state)) of differences
        # between each combination of states.
        ...
        nbrs *= (np.any((diffs[..., 0] == 0)*(diffs[..., 1] != 0), axis=2)
                 is False)
        ...
        ham[nbrs] = ts
        # Find whether each dot in each state pair gained or lost an electron
        gainOrLose = np.where(diffs[..., 0] > 0, 1, 0)
        gainOrLose[diffs[..., 0] < 0] = -1
        # Find whether (quasi-)dots went from odd->even (1) or even->odd (0)
        parityChange = np.array([
            states[i, :, 0] % 2 - states[..., 0] % 2
            for i in range(nStates)
            ])
        # Index of whether tunneling for dots involved u or v coherence factor
    ...
    def state_from_index(self, i, N=None):
        """Returns state vector given its flattened index in Hilbert space."""
        states = self.__choose_states(N)
        return states[i, ...]

    def get_num_op(self, name, N=None):
        """Returns number operator for object 'name'."""
        if not self.__isFinalized:
            self.finalize()
        states = self.__choose_states(N)
        return np.diag(np.sum(states[:, self.__sys['name'] == name, 0],
                              axis=1))

    def cp_stability_diagram(self, probeName, leverArms, gates,
                             N=None, T=0, nLevels=None, tempTol=0.1,
                             sparse=False, removeJumps=False, flipAxes=False,
        ...
        solves for the system ground state at each value of 2 swept gate
        voltages, and produces a colormap of the parametric capacitance, the
        GS energy, and the number expectation values of the two objects whose
        voltages are swept.

        Parameters:
        probeName (str): Dot whose parametric capacitance is to be plotted.
            Must be one of the dots in 'gates' whose voltage is swept.
        leverArms (list of 2 floats): Contains lever arm of probed dot to the
            other dot whose voltage is swept in its first index, and the other
            swept voltage's lever arm to the probed dot in the second index.
        gates (dict): Contains voltage settings for all (non-lead) objects,
            two of which should be a list/array of reduced voltages to be swept
            over.

        Keyword Arguments:
        N (int): Total charge in system, or total charge per dot if leads are
            present. If set equal to none, the largest N for which states have
            already been calculated is used.
        T (float): k_b*T, in relative units. Thermal expectation values are
            used if T!=0 including nLevels additional states, or a number
            determined relative to the system size by default. If T==0, only
            the ground state is considered.
        nLevels (int): Number of levels (ground & excited) to include in
            thermal averages, if T != 0.
        tempTol (float between 0 and 1): Thermal probability cutoff for states
            considered, above which an exception is raised.
        sparse (bool): Whether or not to use sparse, or regular
            diagonalization.
        removeJumps (bool): Whether or not to take large jump discontinuities
            in C_p and set them equal to zero. Sets any value more than two
            standard deviations away from the mean equal to zero.
        flipAxes (bool): Whether or not to flip x and y axes of produced
            colormaps. Otherwise, probed dot gate voltage is placed on the x
            axis.
        plotparams (kwargs): Keyword arguments to be passed to pcolormesh when
            plotting colormaps.
        """
        if not self.__isFinalized:
            self.finalize()
        ...
        gvar = np.array([n for n, g in gates.items()
                         if not np.isscalar(g) and len(g) > 1])
        if len(gvar) != 2:
        ...
        if any([n not in gates for n in sys['name'][sys['isLead'] is False]]):
            raise Exception("Gate voltages must be supplied for all dots in "
                            + "the system!")
        ...
        # If temperature != 0, make sure more than one eigenvalue of
        # hamiltonian is obtained.
        if T != 0 and nLevels is None:
        ...
        for i, g0 in enumerate(gates[probeName]):
            ...
            for j, g1 in enumerate(gates[coupledDot]):
                # Calculate and generate Hamiltonian for each voltage combo
                gs = dict(gates)
                gs.update({probeName: g0+leverArms[0]*g1,
                ...
                if T != 0:
                    ...
                    if np.exp(-e0[-1]/T)/Z > tempTol:
                        raise Exception(
                            ...
                            + "than nLevels = "
                            + "{n} should be included.".format(n=nLevels)
                            )
            ...
            if self.verbose:
                print("Finished row {row}/{totRows} in: {t}s."
                      .format(row=i+1, totRows=len(gates[probeName]),
        ...
        if contoured:
            ...
            thickness = 4
            plt.contour(xAxis[0:-1], yAxis[0:-1], cpos, 1, colors=['black'],
                        antialiased=True, linewidths=thickness)
            plt.contour(xAxis[0:-1], yAxis[0:-1], cneg, 1, colors=['red'],
                        antialiased=True, linewidths=thickness)
            ...
            cp = effLever1*(np.diff(ns0, axis=0)/dx
...
def main():
    ...
    system.add_dot(350, name='dot0', degeneracy=1, orbitals=0, spin=True,
                   isSC=False)
    system.add_dot(100, name='dot1', degeneracy=1, orbitals=70, spin=True,
                   isSC=True, u=np.sqrt(0.7))
    ...
    system.get_states(N)
    npoints = 200
    gates = {'dot0': np.linspace(0, N, npoints),
             'dot1': np.linspace(0, N, npoints)}
    leverArms = [0.2, 0.05]
